...
/* 
  ...
  const sharePost = async (post) => {
    ...
    // ================= 1. INSTANT SHARE (NO AWAIT BEFORE THIS) =================
    let shareUrl = post?.shareUrl || window.location.href;
    let shareTitle = "SocialGist";
    let shareText = post?.description || "";

    try {
      // MUST be executed immediately in gesture context
      if (navigator.share) {
        navigator.share({
          title: shareTitle,
          text: shareText,
      ...
      console.error("Share trigger failed:", err);
    }

    // ================= 2. BACKGROUND TASKS (DO NOT BLOCK SHARE) =================
    (async () => {
      try {
        // update share count
        ...
        // edge function
        const { data } = await supabase.functions.invoke("share-post", {
          body: {
            postId: post.id,
        ...
        // generate image (background only)
        ...
        // optional: advanced share (only if still needed later)
        if (navigator.canShare?.({ files: [file] })) {
          console.log("File ready for future sharing");
        }
      } catch (err) {
        console.error("Background share processing failed:", err);
      }
    })();
  };
  // ================= LOADING =================
                  ...
                  >
                    {profileImages[post.user_id] ? (
                      ...
                      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-violet-600 to-fuchsia-500 flex items-center justify-center text-white font-bold text-lg shadow-sm">
                        {(post.profile_name || "A").charAt(0).toUpperCase()}
                      </div>
                    )}
                  </button>
